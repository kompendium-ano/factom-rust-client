...
//! 
//! Documentation for all api methods are located on the 
//! [Factom struct page](struct.Factom.html)
...
The heights returned provide a lot of information regarding the state of factomd,
 ...
 most recent block, which could not be complete, and still being built. 
 The heights mean as follows:
...
network. This block is not yet complete, but all transactions submitted will go 
into this block (depending on network conditions, the transaction may be delayed 
into the next block)
...
* entryheight : The height at which the local factomd node has all the entries. 
If you added entries at a block height above this, they will not be able to be 
retrieved by the local factomd until it syncs further.

A fully synced node should show the same number for all, (except between minute 
0 and 1, when leaderheight will be 1 block ahead.)
# Example
```
use factom::*;
